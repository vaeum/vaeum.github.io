...
Хорошо, давайте перейдем к реализации полного экрана карты.
Как обычно, мы сначала создадим пользовательский класс для контроллера карты.
В навигаторе проекта щелкните правой кнопкой мыши на папку **FoodPin** и выберете
...
Опять же, мы сначала импортируем структуру **MapKit**. Вставьте следующую строку
...
Перейти к **Interface Builder** и выберете контроллер, на котором расположена
...
так, что я не буду вдаваться в подробности. Мы превращаем адрес в координату
...
используем метод **selectAnnotation** для отображение выности.
...
Было бы замечательно, если мы можем показать эскиз ресторана в выноске пина?
...
и создать свой собственный вид аннотации и встроить изображения ресторана.
...
распространённых моделей в развитии **IOS**. Теперь я предлагаю, что у вас есть
...
виде синей точки на картах. Даже если мы не включили приложение, чтобы отобразить
текущее местоположение, мы не хотим, изменить свой вид аннотации. В самом 
начале кода, мы проверяем, если объект аннотаций является своего рода
**MKUserLocation**. Если да, то мы просто возвращаем **nil** и карта сохранит
и отобразит местоположение с помощью синей точки.

Из соображений производительности, предпочтительно, чтобы повторно использовать
существующий вид аннотаций вместо того, чтобы создавать новую. Карта достаточна
...
(withIdentifier:)** методу, чтобы увидеть, если все неиспользуемые виды доступны.
...
**leftCalloutAccessoryView**, который является видом, отображающийся на левой
стороне выноски пина.

Круто! Нажмите кнопку **Run** и запустите приложение. Выберете ресторан и нажмите
**"Мини-карту"**, полноэкранном виде, вы должны увидеть эскиз в выноске пина.

![Отображение изображение в аннотации](https://monosnap.com/file/RJx9szG6nxKPWTLEQGtge6q8i7AZvM.png)
...
* **showTraffic** - показывает любой высокий трафик на ваш взгляд карте
